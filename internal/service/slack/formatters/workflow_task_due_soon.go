...
func FormatWorkflowTaskDueSoonMessage(
	...
	greeting := "This is a reminder that one of your workflow tasks is due soon."
	if name := strings.TrimSpace(userName); name != "" {
		greeting = fmt.Sprintf("Hi %s, this is a reminder that one of your workflow tasks is due soon.", name)
	}
	...
	blocks := []slack.Block{
		slack.NewHeaderBlock(slack.NewTextBlockObject(slack.PlainTextType, "Task Due Soon", true, false)),
	...
	fallback := fmt.Sprintf("Task due soon: %s (%s)", stepTitle, workflowTitle)

...
Use extend_filter with target_filter_id only when one of this plan's own dashboards has exactly that label-set. Otherwise use new_filter with a short descriptive proposed_filter_name. Global dashboards are listed only to avoid duplicate names; never extend them.

Only reference control_key and label_set_hash values present in the input. Reasoning must state both why the evidence satisfies the control and why it belongs to this system. Provide confidence from 0 to 1.`
		...
		"properties": map[string]any{
			"mappings": map[string]any{
				...
				"items": map[string]any{
					"type":                 "object",
					"additionalProperties": false,
					"required":             []any{"control_key", "label_set_hash", "action", "confidence", "reasoning"},
					"properties": map[string]any{
